...
i=input("请输入获取数:")#43335
j=1
try:
	file=urllib.request.urlopen('http://xapi.xunma.net/Login?uName=test_qq&pWord=12345678&Developer=8pRH3yF8n2Vz23Hfd%2b%2bHuQ%3d%3d')
...
while j<=int(i):
	...
	try:
		file1=urllib.request.urlopen('http://xapi.xunma.net/getPhone?ItemId=3658&token='+token)
	except error.HTTPError as e:
		print (e.code)
	except error.URLError as e:
		print (e.reason)
	...
	if iphone[:5] != 'False' and len(iphone) > 11:
		...
		#f2.close()
	try:
		releaseUrl = 'http://xapi.xunma.net/releasePhone?token='+token+'&phoneList='+phone+'-3658;'
	...
	if ufile[:5] == 'False':
		...
		print('重新获取token...')
		try:
			rfile=urllib.request.urlopen('http://xapi.xunma.net/Login?uName=test_qq&pWord=12345678&Developer=8pRH3yF8n2Vz23Hfd%2b%2bHuQ%3d%3d')
		except error.HTTPError as e:
			print (e.code)
		except error.URLError as e:
			print (e.reason)
